...
class TorchNN_train(nn.Module):
  ...
  def __init__(self) -> None:
    super().__init__()
    self.w00 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)
    self.b00 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)
    self.w01 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)

    self.w10 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)
    self.b10 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)
    self.w11 = nn.Parameter(torch.tensor(rd.random()), requires_grad=True)
